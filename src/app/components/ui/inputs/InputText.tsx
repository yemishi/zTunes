...
export default function InputText({
  ...
  const fnc = () => {
    ...
    return onblur ? (onblur(value), setIsInput(false)) : onBlurDefault();
  };
  const Input = (
    <textarea
      onBlur={fnc}
      ...
      value={value}
      onChange={(e) => {
        setError(false), setValue(e.target.value);
      }}
      className="bg-transparent outline-none  max-w-full line-clamp-2  resize-none text-center"
    />
  );
  ...
  return (
    <div
      ...
      } ${errorClass} ${defaultTextSize} ${defaultFont} text-3xl text-center
  mt-3`}
    >
