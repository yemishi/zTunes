...
export default function ArtistAlbums({ artistId }: { artistId: string }) {
    ...
    const albums = data ? data.filter(
        (album: BundleType) => album.type.toLowerCase() === "album"
    ) : [];

    const singles = data ? data.filter(
        (album: BundleType) => album.type.toLowerCase() === "single"
    ) : [];
    return <>
        {data && <ErrorWrapper error={data.error} message={data.message}>
            {!!albums.length && (
