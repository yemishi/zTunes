"use client";
import CardSearch from "../../search/components/cardSearch";
import { SearchType } from "../../search/page";
...
export default function AddSongToPlaylist({
  ...
  const { values, isFetchingNextPage, hasNextPage, ref, isLoading, isError, error, fetchNextPage } =
    useScrollQuery<SearchType>({
      queryKey: ["search-songs", search],
      url: `/api/search?onlySongs=true&q=${search}&username=${username}&playlistId=${playlistId}`,
    });

  return (
    <div className="w-[380px] h-[550px] md:h-[900px] md:w-[450px] flex flex-col gap-2 rounded-lg overflow-auto bg-black-450">
        ...
        <>
          {!isFetchingNextPage && hasNextPage && (
            <Button
              ...
              className="self-center mt-auto mb-4 rounded-md bg-white font-semibold text-black-300"
            >
              More
            </Button>
          )}
...
const SongSearchSkeleton = () => (
  <div className="flex items-center p-2 !justify-between w-[380px] md:w-[430px] ">
