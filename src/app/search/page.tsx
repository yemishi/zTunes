...
async function fetchData(q: string, username: string) {
  ...
  const data: SearchType[] = await fetch(
    `${process.env.URL}/api/search?q=${decodeURI(q)}&username=${username}`,
    {
      cache: "no-cache",
    }
  ).then((res) => res.json());
...
async function getHistoric(username: string) {
  const data: SearchType[] = await fetch(
    `${process.env.URL}/api/search?username=${username}&getHistory=true`
  ).then((res) => res.json());
...
export default async function Search({
  ...
  if (!q && username) {
    ...
    return (
      <>
        {history.length > 0 && (
          <HistoricSearch username={username} historic={history} />
        )}
      </>
    );
  }
