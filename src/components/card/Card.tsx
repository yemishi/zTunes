...
export default function Card({
  ...
  const subClass = "hidden md:block font-light text-gray-400 first-letter:uppercase text-sm line-clamp-2 ml-1";
  return (
    <div
    ...
    >
      <Link href={`${url}`} className="flex flex-col gap-2">
        <Image alt={alt} src={coverPhoto} className="w-full h-40 rounded-xl md:h-52 md:rounded-lg object-cover" />
        <span className="font-poppins first-letter:uppercase truncate ml-1">{title}</span>
      </Link>
